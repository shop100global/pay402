...
interface Pay402Config {
    ...
    network?: string;
    routes: Pay402Route[];
    tools?: any[]; // For bulk upload if different from routes, but user said "update only when the developer adds a new route to the config"
}
...
interface Pay402Request extends Request {
    ...
    payment_receipt?: any;
    // We don't need to override 'route' here as it's already in Request, 
    // but we might want to access our custom route info if we attached it.
    // For now, let's leave it as is, but remove the conflicting 'route' property if it was causing issues.
    // The error was: Property 'route' is optional in type 'Pay402Request' but required in type 'Request'.
    // We won't redeclare 'route' here.
}

// Cache for routes hash
let lastUploadedRoutesHash: string | null = null;

// Function to dynamically extract all route parameters from URL
const extractAllParams = (path: string) => {
...
const extractParamsFromRoute = (req: Request) => {
    return Object.keys(req.params).length > 0 ? req.params : extractAllParams(req.path);
};

const buildRequestObject = (req: Request): Pay402Context => {
    const my_request: Pay402Context = {
        body: req.body || {},
        route: {
...
async function getRate({ from, to, amount }: { from: string; to: string; amount?: string }, apiKey: string) {
    ...
    try {
        const response = await fetch(
            `${RATES_API}/api/price/${from}?currency=${to}`,
            {
                method: "GET",
                headers: {
...
const uploadRoutesToRemoteServer = async (config: Pay402Config) => {
...
const verifyPay402 = async (payment: string, apiKey: string) => {
...
export const withPay402 = (config: Pay402Config) => {
    ...
    return async (req: Request, res: Response, next: NextFunction) => {
        ...
        (req as any).my_request = my_request;

        const constructedRoutes: { [key: string]: any } = {};
        ...
        for (let i = 0; i < config.routes.length; i++) {
            ...
            if (route.path === my_request.route.path && route.method === my_request.route.method) {
                if (req.headers['x-pay402']) {
                    const payment = req.headers['x-pay402'] as string;
                    try {
                        const receipt = await verifyPay402(payment, config._100PayApiKey);
                        (req as any).payment_receipt = receipt;
        ...
        paymentMiddleware(...params)(req, res, next);
    };
};
